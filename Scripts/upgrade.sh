...
sfctl application provision --application-type-build-path testappApplication 
if [ $? -ne 0 ]; then
    echo "Application type registration failed."
    exit 1
fi
sfctl application upgrade --app-id fabric:/testappApplication --app-version ${version} --parameters [] --mode "Monitored"
if [ $? -ne 0 ]; then
